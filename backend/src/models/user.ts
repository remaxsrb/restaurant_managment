...
const User = new Schema(
  {
    ...
    password: { type: String, required: true, match: /^(?=.*[A-Z])(?=.*[a-z]{3,})(?=.*\d)(?=.*[\W_]).{6,10}$/ },
    email: { type: String, required: true, unique: true, match: /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,})+$/ },
    ...
    status: { type: String, default: "pending", enum: ["pending, approved, blocked, banned"] },
